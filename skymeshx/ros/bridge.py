...
Topics published (MAVLink → ROS2):
    /skymeshx/{id}/attitude      geometry_msgs/Vector3
    /skymeshx/{id}/gps           sensor_msgs/NavSatFix
    /skymeshx/{id}/battery       sensor_msgs/BatteryState
    /skymeshx/{id}/mode          std_msgs/String
    /skymeshx/{id}/telemetry     std_msgs/String (JSON)

Topics subscribed (ROS2 → MAVLink):
    /skymeshx/{id}/cmd/mode      std_msgs/String
    /skymeshx/{id}/cmd/arm       std_msgs/Bool
    /skymeshx/{id}/cmd/goto      geometry_msgs/Point (x=lat,y=lon,z=alt)
...
class ROS2Bridge:
    def __init__(self, drone, node_name: str = None):
        self._drone     = drone
        self._node_name = node_name or f"skymeshx_{drone.id}"
        self._node      = None
        self._thread    = None
        self._running   = False
...
if _ROS2_AVAILABLE:
    class _DroneNode(Node):
        ...
        def __init__(self, name: str, drone):
            ...
            # Publishers
            self._pub_att  = self.create_publisher(Vector3,      f"/skymeshx/{did}/attitude",  10)
            self._pub_gps  = self.create_publisher(NavSatFix,    f"/skymeshx/{did}/gps",       10)
            self._pub_bat  = self.create_publisher(BatteryState, f"/skymeshx/{did}/battery",   10)
            self._pub_mode = self.create_publisher(String,       f"/skymeshx/{did}/mode",      10)
            self._pub_tel  = self.create_publisher(String,       f"/skymeshx/{did}/telemetry", 10)

            # Subscribers
            self.create_subscription(
                String, f"/skymeshx/{did}/cmd/mode",
                lambda msg: drone.set_mode(msg.data), 10
            )
            self.create_subscription(
                Bool, f"/skymeshx/{did}/cmd/arm",
                lambda msg: drone.arm() if msg.data else drone.disarm(), 10
            )
            self.create_subscription(
                Point, f"/skymeshx/{did}/cmd/goto",
