...
- Conversion functions in [`skymeshx/ros/px4_bridge.py`](skymeshx/ros/px4_bridge.py): `ned_to_enu()`, `enu_to_ned()`, `frd_to_flu()`
...
- MUST use [`acquire_ros()`](skymeshx/ros/context.py) / [`release_ros()`](skymeshx/ros/context.py) - never call `rclpy.init()` directly
...
- Top-level [`skymeshx/__init__.py`](skymeshx/__init__.py) uses lazy imports to avoid hard dependencies
...
# CLI
skymeshx connect               # Default: tcp:127.0.0.1:5762
skymeshx status --port tcp:127.0.0.1:5760
...
3. Local absolute (`from skymeshx.core.fsm import StateMachine`)
